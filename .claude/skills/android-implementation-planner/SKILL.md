...
承認後、この計画を GitHub Issue として記録します（`gh issue create`）。
```

ユーザーが明示的に承認するまでコード実装に入らない。`modify:` で修正要望が来た場合は計画を更新したうえで再度 WAITING FOR CONFIRMATION に戻る。Issue 化はあくまで「承認済みの計画を残す」ためなので、暫定版を立てない。

### 9. 承認後、計画を Issue として記録する

ユーザーから `yes`（または同等の承認）が返ったら、`gh issue create` を 1 回だけ呼んで計画を Issue 化する。コード実装より前に行うこと。Issue が残ることで、ブランチ／PR が消えても「なぜこの実装か」が辿れる状態になる。

```bash
gh issue create \
  --title "実装計画: <計画書冒頭のタイトル>" \
  --label enhancement \
  --body-file -
```

`--body-file -` を使い、計画書全文（`# 実装計画: ...` から `**WAITING FOR CONFIRMATION**` 直前まで）を標準入力で渡す。Markdown 構造は GitHub 上でそのままレンダリングされる。

**Issue 本文には必ず Decision Log セクションを含めること**。「採用 API・パターン」表の直後に `## 検討した代替案 (Decision Log)` を置き、表で「不採用」扱いした各候補について以下を 1〜3 行で残す:

- **候補**: API / ライブラリ / パターン名
- **却下理由**: 非推奨 / minSdk 不適合 / プロジェクト規約と不整合 / 過剰機能 など、決定的な 1 点
- **根拠**: 引いた `kb://...` URL や事実

これは「なぜ A でなく B か」を将来の読者（自分含む）に渡すための記録。表の「代替案（不採用理由）」列は 1 行の要約、Decision Log は出典付きの詳細、という役割分担。

ラベル選定:

- 既定は `enhancement`
- 既存バグの修正計画（クラッシュ修正・回帰修正・セキュリティパッチ等）は `bug` を選ぶ
- 依存ライブラリ更新・Gradle / Spotless 設定変更などは `dependencies` を選ぶ
- 該当なしならラベルなしで作成する（`--label` フラグ自体を外す）

新規ラベルは作らない。既存運用に合わない場合はラベルなしで投稿し、運用判断はユーザーに委ねる。

実行後、出力された Issue URL と **番号** をユーザー向けに明示する。次工程の `android-implementation-executor` が PR 本文に `Closes #<番号>` を埋め込んで Issue を自動クローズするため、会話文脈に番号を残すことが重要。

```
計画を Issue として記録しました: https://github.com/CASL0/mediastoreexplorer/issues/<番号> (#<番号>)
次に `android-implementation-executor` で実装に進む場合、PR 本文には `Closes #<番号>` が自動で含まれます。
```

`gh issue create` が失敗した場合（`gh` 未認証 / ネットワーク / ラベル不存在など）は、計画書本文を会話に残したまま停止し、失敗内容と必要な復旧手順をユーザーに伝える。Issue なしで先に進む選択肢もユーザーに提示する（記録は失われるが実装は止めない）。

### 10. Issue 作成後の改訂を反映する（ユーザー明示指示のみ）

Issue 作成後にチャットで計画が変わった場合（例: 「やっぱり A じゃなくて B にしたい」「Phase 3 不要だった」「採用ライブラリを差し替える」）、ユーザーが **明示的に「Issue を更新して」「#<n> に反映して」等の指示を出したときだけ** Issue を編集する。

Claude 側から先回りで提案しない。「議論っぽい発言」を勝手に Issue 反映するとノイズが増え、本物の決定との区別が付かなくなるため。

#### 改訂の手順

1. **現本文を取得**:
   ```bash
   gh issue view <番号> --json body --jq .body > /tmp/issue_<番号>.md
   ```

2. **差分を本文に直接書き込む**。**新しい内容で古い内容を上書きせず、取り消し線で残す**:

   - 行内の単語差し替え:
     `~~Library A~~ **Library B** *(2026-05-28 更新: A は API 33 で deprecated、B が現行推奨)*`
   - 段落・テーブル行・Phase の差し替え:
     ```markdown
     ~~### Phase 3: 旧アプローチ~~
     ~~- 編集ファイル: ...~~
     ~~- 主要な変更: ...~~

     ### Phase 3: 新アプローチ *(2026-05-28 改訂)*
     - 編集ファイル: ...
     - 主要な変更: ...
     ```
   - Decision Log への追記:
     却下されたばかりの旧採用案を Decision Log に **追加する**（最初は採用していたが後に却下された候補も、却下理由付きで Decision Log に残る）

3. **末尾に改訂履歴を append**（無ければ新規追加）:

   ```markdown
   ## 改訂履歴

   - **2026-05-28**: 採用ライブラリを `Library A` → `Library B` に変更（理由: A の API 33 deprecated）。Decision Log に Library A を追加
   - **2026-05-30**: Phase 3 を削除（理由: 既存 helper で代替可能と判明）
   ```

4. **Issue を更新**:
   ```bash
   gh issue edit <番号> --body-file /tmp/issue_<番号>.md
   ```

5. **更新内容を 1 行で会話に報告**:
   ```
   #<番号> を更新しました（改訂 N 件追加: 採用ライブラリ変更 / Phase 3 削除）
   ```

#### 改訂時の禁止事項

- **取り消し線を経由せず本文を書き換えない**。「初版」と「変更点」が判別できなくなる
- **承認待ち (`WAITING FOR CONFIRMATION`) を経由せず破壊的な再構成をしない**。表全体の組み替えや Phase の大規模再編は、改訂前に計画書としてチャットに出し、ユーザー承認を取ってから Issue に反映
- **タイトルは原則変えない**。実装範囲が根本的に変わった場合のみタイトル変更を提案する（その場合は新規 Issue を立てる選択肢も提示）
...
## 検討した代替案 (Decision Log)
<採用 API 表で「不採用」になった各候補について、却下理由と根拠を残す。
将来「なぜ A でなく B か」を辿れるようにするための記録。1 候補 1〜3 行で簡潔に。>

- **`LegacyXxx`** — 却下: API 30 で deprecated、`androidx.foo.Bar` が公式置き換え。出典: kb://android/...
- **`手書き ContentResolver`** — 却下: MediaStore 便利 API があり再発明になる。プロジェクト規約 (`docs/architecture.md`) の "DataSource は ContentResolver アダプターに薄く" と整合しない
- **<その他不採用候補>** — 却下: <一行>
...
<!-- 以下は Issue 化された後、ユーザー明示指示で改訂が入ったときだけ追加される -->
## 改訂履歴
- **YYYY-MM-DD**: <変更内容と理由を 1 行>

**WAITING FOR CONFIRMATION**: この計画で進めますか？ (yes / modify: <内容> / different approach: <代替案>)
承認後、この計画を GitHub Issue として記録します（`gh issue create --label enhancement`）。
```
...
- **コードを書かない。** 計画のみ。実装への移行はユーザー承認後、`android-implementation-executor` で行う
- **未承認の計画を Issue 化しない。** ユーザーが `yes` を返す前に `gh issue create` を呼ばない。承認前の修正サイクルは会話内で完結させ、Issue は確定した初版にのみ立てる
- **改訂をユーザーに無断で行わない。** Issue 作成後、チャットで議論が起きても、ユーザーが「Issue を更新して」「#<n> に反映して」等の明示指示を出すまで `gh issue edit` を呼ばない
