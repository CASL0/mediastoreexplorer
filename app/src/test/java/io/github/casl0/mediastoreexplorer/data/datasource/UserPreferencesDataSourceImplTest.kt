...
class UserPreferencesDataSourceImplTest {
    ...
    private lateinit var dataSource: UserPreferencesDataSourceImpl
    ...
    fun setUp() {
        ...
        dataSource = UserPreferencesDataSourceImpl(dataStore, ioDispatcher)
    }
    ...
    fun 初期状態は_System_と_dynamicColor_true() = runTest {
        val state = dataSource.preferences.first()
    ...
    fun setDynamicColor_false_を渡すと_dynamicColor_に保存される() = runTest {
        ...
        assertEquals(false, dataSource.preferences.first().dynamicColor)
    }
