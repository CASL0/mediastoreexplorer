...
class DownloadsScreenTest {
    ...
    fun パーミッション未付与のとき_パーミッション要求画面のメッセージが表示される() {
        val viewModel = DownloadsViewModel(FakeMediaRepository())
        composeTestRule.setContent {
            DownloadsScreen(viewModel = viewModel, initialPermissionsGranted = false)
        }

        composeTestRule
            .onNodeWithText(context.getString(R.string.permission_downloads_message))
            .assertIsDisplayed()
    }
