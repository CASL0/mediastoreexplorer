...
class AudiosScreenTest {
    ...
    fun パーミッション未付与のとき_パーミッション要求画面のメッセージが表示される() {
        val viewModel = AudiosViewModel(FakeMediaRepository())
        composeTestRule.setContent {
            AudiosScreen(viewModel = viewModel, initialPermissionsGranted = false)
        }

        composeTestRule
            .onNodeWithText(context.getString(R.string.permission_audios_message))
            .assertIsDisplayed()
    }
